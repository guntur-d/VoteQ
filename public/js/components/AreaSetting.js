// /public/js/components/AreaSetting.js
  ...
  oninit() {
    ...
    m.request({
      ...
      if (setting && setting.provinsi && setting.kabupatenKota) {
        ...
        // Fetch kabupaten list to get the name
        m.request({ method: 'GET', url: `/api/kabupatenkota?provinsiCode=${setting.provinsi}` })
          .then(kabupatenList => {
            this.kabupatenList = kabupatenList;
            const kab = kabupatenList.find(k => k.code === setting.kabupatenKota);
