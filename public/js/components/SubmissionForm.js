...
const SubmissionForm = {
  ...
  oninit(vnode) {
    this.resetForm();
    console.log('[SubmissionForm] oninit called', vnode.attrs);
    if (vnode.attrs && vnode.attrs.prefill) {
      console.log('[SubmissionForm] oninit prefill:', vnode.attrs.prefill);
      if (vnode.attrs.prefill.kelurahanDesaCode) {
  ...
  async fetchLastSubmission(kelurahanDesaCode) {
    ...
    try {
      // Try to get more area info from prefill if available
      let prefill = (this.vnode && this.vnode.attrs && this.vnode.attrs.prefill) || {};
      console.log('[SubmissionForm] prefill:', prefill);
      let district = this.district || prefill.district;
      let village = this.village || prefill.village;
      let volunteerId = this.volunteerId || prefill.volunteerId;
      //lets log the query parameters
      console.log('[SubmissionForm] fetchLastSubmission prefill:', prefill);    
      // fallback to empty string if undefined
      district = district || "";
      village = village || "";
      volunteerId = volunteerId || "";
      const params = [];
      if (kelurahanDesaCode) params.push(`kelurahanDesaCode=${encodeURIComponent(kelurahanDesaCode)}`);
      if (district) params.push(`district=${encodeURIComponent(district)}`);
      if (village) params.push(`village=${encodeURIComponent(village)}`);
      if (volunteerId) params.push(`volunteerId=${encodeURIComponent(volunteerId)}`);
      const query = params.length ? `?${params.join('&')}` : '';
      console.log('[SubmissionForm] fetchLastSubmission query:', query);
      const token = localStorage.getItem("token");
      const data = await m.request({
        method: "GET",
        url: `/api/submissions/mine${query}`,
        headers: { Authorization: `Bearer ${token}` }
      });
      console.log("[SubmissionForm] fetchLastSubmission response:", data);
      ...
      console.error("[SubmissionForm] fetchLastSubmission error:", e);
